...
(define Politician%
  (class* object% (Toy<%>)
    ...
    ;; EFFECT: Updates the politician to the state that should follow the given
    ;;         state affter mouse events. certain attributes like vx, vy, prev-mx,
    ;;         prev-my are set. 
    ;; EXAMPLE: see test cases for example
    ...
    (define/public (after-move mx my)
      (set! prev-mx mx)
      (set! prev-my my)
      (set! vx (update-vx mx my))
      (set! vy (update-vy mx my)))
    ...
    (define/public (after-key-event kev) 49)
    ;===========================================================================
    ;; after-tick : -> Void
    ;; RETURNS: this politician with location&face updated by one tick
    ;; EXAMPLE: see test cases for example
    ;; Strategy: update field based on mouse locations
    (define/public (after-tick)
      (let [(repel? (repel-politician? prev-mx prev-my))]
        (set! x (+ x (* vx (if repel? REPEL-SPEED SPEED))))
        (set! y (+ y (* vy (if repel? REPEL-SPEED SPEED))))
        (set! face1? (if repel? (not face1?) face1?))))
    ;===========================================================================
...
(begin-for-test
  (local
    ((define p0 (new Politician%))
     (define p1 (new Politician% [x INIT-X][y INIT-Y] ))
     (define p2 (new Politician%))
     )
    (send p0 after-button-up INIT-X INIT-Y)
    (send p0 after-drag INIT-X INIT-Y)
     (check-equal?
     (send p0 toy-x) INIT-X
     "if mouse position is the center position, x should remain still")
    (check-equal?
     (send p0 toy-y) INIT-Y
    ...
    (send p1 after-button-up INIT-X INIT-Y)
    (send p1 after-drag INIT-X INIT-Y)
    (check-equal?
     (send p0 add-to-scene EMPTY-CANVAS)
     (send p1 add-to-scene EMPTY-CANVAS)
     "p0 and p1 should be same in appearance")
    (send p1 after-drag INIT-X INIT-Y)
    (send p2 after-button-up INIT-X INIT-Y)
    (send p1 after-tick)
    (send p2 after-tick)
    (check-equal?
     (send  p1  add-to-scene EMPTY-CANVAS)
     (send  p2  add-to-scene EMPTY-CANVAS)
     "p1-afte-tick and p1-after-tick should be same in appearance"))
    
   
  (local
    ((define p0 (new Politician% [x 10][y 10]))
     (define p4 (new Politician% [x 10][y 10]))
     )
    (send p0 after-key-event "s")
    (send p0 after-button-down 20 20)
    (check-equal?
     (send p0 for-test:get-vx)
     (send p0 for-test:get-vy)
     "if direction vector's slope is exactly 1, vx should equal to vy")
    (send p0 after-button-down 5 5)
    (check-equal?
     (send p0 for-test:get-vx)
     (send p0 for-test:get-vy)
     "if direction vector's slope is exactly 1, vx should equal to vy")
    (send p4 after-button-down 10 500)
    (send p4 after-tick)
    (check-equal?
     (send p4 toy-x) 10
     "p4-after-tick should not change on x")
    )

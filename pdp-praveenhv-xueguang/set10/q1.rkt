...
(define Metatoy%
  (class* object% (Metatoy<%>)
    ...
    ;; after-tick : -> Void
    ;; GIVEN: No arguements
    ;; EFFECT: updates this Metatoy to the state it should have following a tick.
    ;; DETAILS: The change in all the stateful widgets in the
    ;;          Metatoy after a tick is the change in the Metatoy.
    ;; STRATEGY: Use HOF for-each on the toys (SWidget's) in this Metatoy
    ...
    ;;GIVEN: the button up event location
    ;;EFFECT: toys present in the list of toys of the metatoy are updated to the
    ;;        state that should follow the current state due to mouse event.
    ;;Strategy:  Use HOF for-each on the toys (SWidget's) in this
    ;;           Metatoy
...
;; Strategy: use constructor template for Container
(define (run rate)
  (local

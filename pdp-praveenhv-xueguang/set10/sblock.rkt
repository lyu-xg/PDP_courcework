...
(begin-for-test
  (local
    ((define block1 (new SBlock% [x INIT-X] [y INIT-Y]))
     ...
     (define block3 (new SBlock% [x INIT-X] [y INIT-Y]))
     (define block4 (new SBlock% [x INIT-X] [y INIT-Y]))
     )
    ...
    (send block1 after-drag 440 310)
    (send block1 after-button-down 0 0)
    (check-equal? (send block1 sblock-x) INIT-X
                  "when not selected, block should not be draged")
    (check-equal? (send block1 add-to-scene EMPTY-CANVAS)
                  (send block2 add-to-scene EMPTY-CANVAS)
                  "block1 and block2 should be identical in appearance")
    (send block1 after-button-down INIT-X INIT-Y)
    (send block2 after-button-down INIT-X INIT-Y)
    (check-equal? (send block1 add-to-scene EMPTY-CANVAS)
                  (send block2 add-to-scene EMPTY-CANVAS)
                  "block1 and block2 should be identical in appearance")
    (send block1 after-drag 440 310)
    (check-equal? (send block1 sblock-x) 440
                  "after-drag should drag the block-x to 440")
    (check-equal? (send block4 intersected-blocks (list block3))
                  (list block3) "block1 and block2 should be intersected")
    (send block1 after-button-up INIT-X INIT-Y)
    (send block1 after-drag 300 400)
    (check-equal? (send block1 sblock-x) 440
                  "after button up, drag should not change position")
    (send block4 after-tick)
    (send block4 after-key-event "a")
    (send block4 after-move 300 200)
    (check-equal? (send block4 sblock-x) INIT-X
                  "position should not change after irrelavent events")
    ))

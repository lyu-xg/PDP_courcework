...
(define Throbber%
  (class* object% (Toy<%>)
    ...
    (define/public (after-drag mx my)
      (if selected?
          (begin
              (set! x (x-after-drag mx))
              (set! y (y-after-drag my))
              (set! selected? true)
              (set! prev-mx mx)
              (set! prev-my my))
          9))
    ...
    (define/public (add-to-scene scene)
          (place-image (create-image ) x y scene))
          
    ;;==========================================================================
    ;;create-image : Boolean -> Image
    ;;GIVEN: a boolean representing whether the throbber is selected.
    ;;RETURNS: a circle image.
    ;;EXAMPLE: (create-image true) -> a circle with a green outline.
    ;;STRATEGY: Use cases on whether the circle is selected.
    (define (create-image )
      (if selected?
...
(begin-for-test
  (local
    ((define th (make-throbber INIT-X INIT-Y)))
